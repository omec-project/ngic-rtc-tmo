...
#define APN_NAME_SET			(0x0080)
#define IP_POOL_IP_SET			(0x0100)
#define IP_POOL_MASK_SET		(0x0200)
#define PRIMARY_DNS_IP_SET      (0x0300)
#define SECONDARY_DNS_IP_SET    (0x0400)
#define TMR_TRSHLD_SET			(0x0500)
#define VOL_TRSHLD_SET			(0x0600)
#define LOG_LEVEL_SET			(0x0700)

#define REQ_ARGS				(S11_MME_IP_SET | \
								S11_SGW_IP_SET | \
								S1U_SGW_IP_SET | \
								APN_NAME_SET | \
								IP_POOL_IP_SET | IP_POOL_MASK_SET | \
								LOG_LEVEL_SET)
...
int s5s8_pgwc_fd = -1;
uint8_t send_cdr = 1;
uint32_t ul_ambr = 0, dl_ambr = 0;

#ifndef SDN_ODL_BUILD
uint8_t node_id[MAX_LEN];
#endif /* SDN_ODL_BUILD */
...
parse_arg(int argc, char **argv)
{
	...
	const struct option long_options[] = {
	  ...
	  {"primary_dns_ip",required_argument, NULL, 'e'},
	  {"secondary_dns_ip",required_argument, NULL, 'f'},
	  {"timer_threshold",required_argument, NULL, 'm'},
	  {"volume_threshold",required_argument, NULL, 'o'},
	  {"send_cdr",   optional_argument, NULL, 'c'},
	  {"node_id",   optional_argument, NULL, 'n'},
	  {"ulambr",   optional_argument, NULL, 'j'},
	  {"dlambr",   optional_argument, NULL, 'k'},
	...
	do {
		...
		c = getopt_long(argc, argv, "d:s:r:g:w:v:u:a:i:j:k:p:e:f:m:o:c:n:l:x:y:", long_options,
		    &option_index);
		if (c == -1)
			break;
		...
		case 'p':
			...
			break;
		case 'e':
			set_primary_dns(optarg);
			args_set |= PRIMARY_DNS_IP_SET;
			break;
		case 'f':
			set_secondary_dns(optarg);
			args_set |= SECONDARY_DNS_IP_SET;
			break;
		case 'm':
			set_timer_trshld(optarg);
			args_set |= TMR_TRSHLD_SET;
			break;
		case 'o':
			set_vol_trshld(optarg);
			args_set |= VOL_TRSHLD_SET;
			break;
		case 'c':
			send_cdr = (uint8_t)atoi(optarg);
			break;
#ifndef SDN_ODL_BUILD
		case 'n':
			memset(node_id, 0, MAX_LEN);
			strncpy((char *)node_id, optarg, strlen(optarg));
			break;
#endif /* !SDN_ODL_BUILD */
...
init_s11(void)
{
	 ...
	 * s11_mme_sockaddr = 0
	 * So: gtpv2c_send(., ., .,
	 *		s11_mme_sockaddr{src_ip, src_port}, .)
	 * Remove:
...
init_cp(void)
{
	...
	/* Initialize Secure STF interface */
	init_sctf();
...
control_plane(void)
{
	...
	if (((spgw_cfg == SGWC) || (spgw_cfg == SPGWC)) && !pcap_reader) {
	...
	if (!pcap_reader) {
			/* ASR- TMOPL VCCCCB-4 CP to support many MMEs
			 * Remove s11 request src_ip check:
			...
			if (
				...
				fprintf(stderr,"Expected S11_MME_IP = %s\n",
				...
				fprintf(stderr, "Discarding packet from %s:%u - ",
						inet_ntoa(s5s8_sgwc_sockaddr.sin_addr),
						ntohs(s5s8_sgwc_sockaddr.sin_port));
				fprintf(stderr,"Expected S5S8_SGWC_IP = %s\n",
		...
		if ((spgw_cfg == SGWC) || (spgw_cfg == SPGWC)) {
			...
			case GTP_CREATE_SESSION_REQ:
				...
				if (ret) {
					...
					/* ASR- Primitive Error handling
					 * return if system error, else send resp */
...
main(int argc, char **argv)
{
	...
	/* Initialize apn, ip_pool element arrays */
	apn_count = ip_count = mask_count = 0;
	pdns_count = sdns_count = 0;
	tmrth_count =  volth_count = 0;

	/* CP Config parse line spacer */
	printf("\nParsing CP configuration...\n");
	...
	init_cp_params();
	init_cp();
	printf("\n\n");       /* CP Config info line spacer */

	printf("apn_count=\t\t%u::", apn_count);
	if (volth_count < apn_count) {
		rte_panic("@%s::Insufficient Volume thresholds for APN LIST:"
				"\n\tapn_count= %u; volth_count= %u\n",
				__func__, apn_count, volth_count);
	}
	for(i = 0; i < apn_count; i++) {
		apn_list[i].ue_pool = &ue_pool_list[i];
		apn_list[i].pdns = &pdns_list[i];
		apn_list[i].sdns = &sdns_list[i];
		apn_list[i].tmr_trshld = &tmr_trshld_list[i];
		apn_list[i].vol_trshld = &vol_trshld_list[i];
		apn_list[i].apn_ambr.ambr_uplink = ul_ambr_list[i];
		apn_list[i].apn_ambr.ambr_downlink = dl_ambr_list[i];

		printf("\n\tapn_list[%d].apn_idx= %u;"
				"\tapn_list[%d].apn_name_label= %s;\n",
				i, apn_list[i].apn_idx,
				i, apn_list[i].apn_name_label);
		printf("\tapn_list[%d].ue_pool->netid= %s",
				i, inet_ntoa(apn_list[i].ue_pool->netid));
		printf("\tapn_list[%d].ue_pool->netmask= %s\n",
				i, inet_ntoa(apn_list[i].ue_pool->netmask));
		printf("\tapn_list[%d].pdns= %s",
				i, inet_ntoa(*apn_list[i].pdns));
		printf("\tapn_list[%d].sdns= %s\n",
				i, inet_ntoa(*apn_list[i].sdns));
		printf("\tapn_list[%d].tmr_trshld= %lu",
				i, *apn_list[i].tmr_trshld);
		printf("\tapn_list[%d].vol_trshld= %lu\n",
				i, *apn_list[i].vol_trshld);
		printf("\tapn_list[%d].apn_ambr.ambr_uplink= %u\n",
				i, apn_list[i].apn_ambr.ambr_uplink);
		printf("\tapn_list[%d].apn_ambr.ambr_downlink= %u\n",
				i, apn_list[i].apn_ambr.ambr_downlink);
	}
	printf("\nCP Main Core on:\t%u\n", rte_get_master_lcore());

	/* ASR- Initialize CUPS response data sructures */
	init_resp_op_id();
