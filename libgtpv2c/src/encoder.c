...
{
	uint16_t enc_len = 0;
	uint16_t tempVal = IPCP_PROTO_CONT_ID;

	/* Populate CSResp PCO params in val*/
	val->header.len = sizeof(pco_flags_t) + sizeof(proto_cont_t) + sizeof(ppp_ipcp_t);
	val->prtCnt.proto_cont_id = htons(tempVal);
	val->prtCnt.proto_cont_len = sizeof(ppp_ipcp_t);
	val->ipcp.len = sizeof(ppp_ipcp_t);
	val->ipcp.len = htons(val->ipcp.len);
	val->ipcp.secondary_dns_type = SECONDARY_DNS_IP;

	/* Encode the PCO block */
	enc_len += encode_ie_header_t(&val->header, buf);
	enc_len += encode_common_type(&val->flags, buf, sizeof(pco_flags_t));
	enc_len += encode_common_type(&val->prtCnt, buf, sizeof(proto_cont_t));
	enc_len += encode_common_type(&val->ipcp, buf, sizeof(ppp_ipcp_t));
