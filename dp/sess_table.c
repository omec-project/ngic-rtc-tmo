...
extern struct rte_hash *rte_pcc_hash;
			...
			struct dp_session_info *data, uint32_t idx)
{
	...
	if (ret < 0)
		rte_panic("Failed to add entry in rte_uplink_hash table");
}
			...
			struct dp_session_info *data, uint32_t idx)
{
	...
	if (ret < 0)
		rte_panic("Failed to add entry in rte_downlink_hash table");
}
			...
			struct ue_session_info *new, uint32_t idx)
{
	...
	ret = rte_hash_add_key_data(rte_adc_ue_hash,
					&key, padc_ue);
	if (ret < 0)
			rte_panic("Failed to add entry in rte_adc_ue_hash table");
...
adc_dns_entry_add(struct msg_adc *data)
{
	...
	*adc = *data;
	...
	ret = rte_hash_add_key_data(rte_adc_hash, &key32,
			adc);
	if (ret < 0){
		RTE_LOG_DP(ERR, DP, "Failed to add entry in rte_adc_hash table");
		return -1;
	}
	return 0;
}
...
get_session_data(uint64_t sess_id, uint32_t is_mod)
{
	...
	if (ret < 0){
		RTE_LOG_DP(ERR, DP, "Failed to add entry in rte_sess_hash table\n");
		rte_free(data);
		return NULL;
	}
		...
		struct session_info *src)
{
	...
	dst->ipcan_dp_bearer_cdr.vol_trshld = src->ipcan_dp_bearer_cdr.vol_trshld;
	dst->ipcan_dp_bearer_cdr.tmr_trshld = src->ipcan_dp_bearer_cdr.tmr_trshld;
	dst->sess_id = src->sess_id;
	dst->client_id = src->client_id;
	dst->service_id = src->service_id;
	dst->ue_context = src->ue_context;
	dst->dp_session = src->dp_session;
	dst->apn_idx = src->apn_idx;
}
		...
		struct session_info *entry)
{
	...
	dp_sess_strct *cli_sess_data = NULL;
	...
	if ((ue_data == NULL) || (ret == -ENOENT)) {
		...
		if (bear_id != DEFAULT_BEARER) {
			 ...
			 */
			RTE_LOG_DP(ERR, DP, "BEAR_SESS ADD Fail:"
					"\n\tDefault bearer not found for sess_id:%u, bear_id:%u\n",
						ue_sess_id, bear_id);
		...
		if (ret < 0) {
			rte_panic("Failed to add entry in rte_ue_hash table");
			return -1;
		}
	...
	time(&rawtime);
	localtime_r(&rawtime, &data->ipcan_dp_bearer_cdr.record_open_time);
	data->ipcan_dp_bearer_cdr.charging_id = entry->sess_id;

	// initialize per APN timer (new APN) and monitor this session
	if (data->apn_idx < MAX_NB_APN) {
		if (!ats_is_apn_timer_initialized(data->apn_idx) &&
			(data->ipcan_dp_bearer_cdr.tmr_trshld != 0))
			ats_init(data->ipcan_dp_bearer_cdr.tmr_trshld, data->apn_idx);

		if (data->ipcan_dp_bearer_cdr.tmr_trshld != 0)
			ats_list_add_to_tail(data->sess_id, data->apn_idx);
	}

	/* Update CLI Session entry for CLI monitoring */
	ret = rte_hash_lookup_data(rte_sess_cli_hash, &ue_sess_id, (void **)&cli_sess_data);
	if ((cli_sess_data != NULL) || (ret != -ENOENT)) {
			rte_hash_del_key(rte_sess_cli_hash, &ue_sess_id);
			rte_free(cli_sess_data);
	}
	/* add UE Session Data into CLI hash*/
	cli_sess_data = rte_zmalloc("cli sess info", sizeof(dp_sess_strct),
			RTE_CACHE_LINE_SIZE);
	if (cli_sess_data == NULL)
		rte_panic("Failed to alloc mem for ue cli session");

	cli_sess_data->ue_addr = data->ue_addr;
	cli_sess_data->sess_id = entry->sess_id;;

	ret = rte_hash_add_key_data(rte_sess_cli_hash, &ue_sess_id, cli_sess_data);
	if (ret < 0) {
		rte_panic("Failed to add entry in rte_sess_cli_hash table");
		return -1;
	}
...
flush_session_records(struct dp_session_info *session)
{
	...
	for (i = 0; i < session->num_dl_pcc_rules; i++) {
		...
		// if (pcc_info != NULL)
			/* SC- Reserved Future Use (RFU) CDR */
	}
...
int update_vol_on_rec_close(struct dp_session_info *session,
		enum cdr_rec_cause closure_cause)
{
	/* UL delta = Total bytes - Last bytes */
	session->ipcan_dp_bearer_cdr.data_vol.ul_bytes_delta =
		(session->ipcan_dp_bearer_cdr.data_vol.ul_cdr.bytes -
		 session->ipcan_dp_bearer_cdr.data_vol.ul_cdr_last.bytes);
	/* UL - Update last bytes count to current total bytes */
	session->ipcan_dp_bearer_cdr.data_vol.ul_cdr_last.bytes =
		session->ipcan_dp_bearer_cdr.data_vol.ul_cdr.bytes;

	/* DL delta = Total bytes - Last bytes */
	session->ipcan_dp_bearer_cdr.data_vol.dl_bytes_delta =
		(session->ipcan_dp_bearer_cdr.data_vol.dl_cdr.bytes -
		 session->ipcan_dp_bearer_cdr.data_vol.dl_cdr_last.bytes);
	/* DL - Update last bytes count to current total bytes */
	session->ipcan_dp_bearer_cdr.data_vol.dl_cdr_last.bytes =
		session->ipcan_dp_bearer_cdr.data_vol.dl_cdr.bytes;

	session->ipcan_dp_bearer_cdr.record_closure_cause = closure_cause;
	return 0;
}

int
create_cdr_report(struct resp_msgbuf *resp, uint64_t sess_id)
{
	...
	resp->cdr_msg.datavolumeFBCDownlink =
		session->ipcan_dp_bearer_cdr.data_vol.dl_bytes_delta;
	resp->cdr_msg.datavolumeFBCUplink =
		session->ipcan_dp_bearer_cdr.data_vol.ul_bytes_delta;

	resp->cdr_msg.ratingGroup =
		session->ipcan_dp_bearer_cdr.rating_group;
	resp->cdr_msg.chargingID =
		session->ipcan_dp_bearer_cdr.charging_id;
	resp->cdr_msg.localSequenceNumber =
		session->ipcan_dp_bearer_cdr.record_seq_number++;
	resp->cdr_msg.timeOfFirstUsage =
		session->ipcan_dp_bearer_cdr.time_of_first_use;
	resp->cdr_msg.recordOpeningTime =
		mktime(&session->ipcan_dp_bearer_cdr.record_open_time);
	time(&rawtime);
	localtime_r(&rawtime, &session->ipcan_dp_bearer_cdr.record_open_time);
	time((time_t *)&resp->cdr_msg.timeOfLastUsage);
	time((time_t *)&resp->cdr_msg.timeOfReport);
	resp->cdr_msg.timeUsage =
		resp->cdr_msg.timeOfLastUsage - resp->cdr_msg.recordOpeningTime;
	resp->cdr_msg.record_closure_cause =
		session->ipcan_dp_bearer_cdr.record_closure_cause;
	resp->cdr_msg.ue_context = session->ue_context;
	resp->cdr_msg.session_info = session->dp_session;
	resp->cdr_msg.apn_idx = session->apn_idx;
	return 0;
}
...
/** RTE_LOG redefinition based on DPDK version */
	...
	if (data->ipcan_dp_bearer_cdr.tmr_trshld != 0)
		ats_list_del_node(data->sess_id, data->apn_idx);
	...
	/* remove entry from ue hash table*/
	if (rte_hash_del_key(rte_ue_hash, &ue_sess_id) < 0)
		return -1;
	...
	rte_free(data->ue_info_ptr);
	rte_free(data);

	/* Delete CLI sess table entry */
	hash_ret = rte_hash_lookup_data(rte_sess_cli_hash, &ue_sess_id, (void **)&cli_sess_data);
	if ((cli_sess_data != NULL) || (hash_ret != -ENOENT)) {
			rte_hash_del_key(rte_sess_cli_hash, &ue_sess_id);
			rte_free(cli_sess_data);
	}
	return 0;
