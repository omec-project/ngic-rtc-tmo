...
unsigned long Options::m_cdrfilesize;
int Options::dryRun;
...
void Options::help()
{
   std::cout << std::endl
             ...
             << "  -f, --cdrfilesize  		CDR file size." << std::endl
...
bool Options::parseJson(){
   ...
   if(doc.HasMember("dealer")){
      ...
      if(!(options & cdrfilesize) && dealerSection.HasMember("cdrfilesize")){
         if(!dealerSection["cdrfilesize"].IsString()) { std::cout << "Error parsing json value: [cdrfilesize]" << std::endl; return false; }
         m_cdrfilesize = atoi(dealerSection["cdrfilesize"].GetString());
         options |= cdrfilesize;
      }
   }
...
bool Options::parseInputOptions( int argc, char **argv )
{
   ...
   struct option long_options[] = {
      ...
      { "cdrfilesize",		required_argument,  NULL, 'f' },
   ...
   while (1)
   {
      c = getopt_long(argc, argv, "hxj:m:n:r:q:p:k:l:s:c:d:f:", long_options, &option_index );
      if (c == -1)
      ...
      switch (c)
      {
         ...
         case 'f': { m_cdrfilesize = atoi(optarg);		           options |= cdrfilesize;             break; }
         case '?':
         {
            switch ( optopt )
            {
               ...
               case 'f': { std::cout << "Option -f (CDR File Size) requires an argument"           << std::endl; break; }
