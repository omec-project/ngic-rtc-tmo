...
int decap_gtpu_hdr_dynamic_seqnb(struct rte_mbuf *m)
{
	void *ret;
	uint8_t *pkt_ptr;
	/* Decap 36 || 40 Bytes: GPDU hdr= 8 || 12B, UDP= 8B, IPv4 hdr= 20B */
	pkt_ptr = (uint8_t *) get_mtogtpu(m);

	ret = rte_pktmbuf_adj(m, GPDU_HDR_SIZE_DYNAMIC(*pkt_ptr) + UDP_HDR_SIZE +
						IPv4_HDR_SIZE + ETH_HDR_SIZE);
	if (ret) {
		/* Prepend sizeof(struct ether_hdr) >>
		 *     ether.c::onstruct_ether_hdr(...) */
		ret = rte_pktmbuf_prepend(m, sizeof(struct ether_hdr));
		if (ret == NULL) {
			RTE_LOG_DP(ERR, DP, "Error: Failed to remove GTPU header\n");
			return -1;
		}
	} else {
		RTE_LOG_DP(ERR, DP, "rte_pktmbuf_adj failed in GTP Decap\n");
		return -1;
	}
	RTE_LOG_DP(DEBUG, DP,
...
int decap_gtpu_hdr_with_seqnb(struct rte_mbuf *m)
{
	...
	/* Decap 40Bytes: GPDU hdr= 12B, UDP= 8B, IPv4 hdr= 20B */
	ret = rte_pktmbuf_adj(m, GPDU_HDR_SIZE_WITH_SEQNB + UDP_HDR_SIZE +
						IPv4_HDR_SIZE + ETH_HDR_SIZE);
	if (ret) {
		/* Prepend sizeof(struct ether_hdr) >>
		 *     ether.c::onstruct_ether_hdr(...) */
...
int decap_gtpu_hdr_without_seqnb(struct rte_mbuf *m)
{
	...
	/* Decap 36 Bytes: GPDU hdr= 8B, UDP= 8B, IPv4 hdr= 20B */
	ret = rte_pktmbuf_adj(m, GPDU_HDR_SIZE_WITHOUT_SEQNB + UDP_HDR_SIZE +
						IPv4_HDR_SIZE + ETH_HDR_SIZE);
	if (ret) {
		/* Prepend sizeof(struct ether_hdr) >>
		 *     ether.c::onstruct_ether_hdr(...) */
...
int encap_gtpu_hdr_with_seqnb(struct rte_mbuf *m, uint32_t teid)
{
	...
	/* Encap 40Bytes: GPDU hdr= 12B, UDP= 8B, IPv4 hdr= 20B */
	pkt_ptr =
...
int encap_gtpu_hdr_without_seqnb(struct rte_mbuf *m, uint32_t teid)
{
	...
	tpdu_len = rte_pktmbuf_data_len(m);
	tpdu_len -= ETH_HDR_SIZE;
	/* Encap 36Bytes: GPDU hdr= 8B, UDP= 8B, IPv4 hdr= 20B */
	pkt_ptr =
		(uint8_t *) rte_pktmbuf_prepend(m,
