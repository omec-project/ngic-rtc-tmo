   ...
   int offline, std::string &cdrPath, std::string &cdrArchPath)
{
   ...
   while(1) {
      ...
      if (status != SGX_SUCCESS || read_bytes == -1) {
         ...
         decrypted_str = (uint8_t *) malloc((size_t)(read_bytes + 1));// +1 for '\0'
...
int unseal_from_file(uint8_t* plain_text, uint32_t* plain_text_len, const char *filename)
{
    sgx_status_t err = SGX_ERROR_UNEXPECTED;
    uint32_t sealedDataSize = 0; 


    SGX_FILE* key_file = NULL;
    key_file  = sgx_fopen_auto_key(filename, "r");

    if (key_file == NULL) { //could not open key file for writing
       mbedtls_printf("ERROR: secure file %s open for read failed\n",filename);
       return SGX_ERROR_UNEXPECTED;
    }


    if (sizeof(uint32_t) != sgx_fread(&sealedDataSize, 1, sizeof(uint32_t), key_file)) {
       mbedtls_printf("ERROR: secure file read failed. file is likely corrupted\n");
       return SGX_ERROR_UNEXPECTED;
    }

    sgx_sealed_data_t *p_sealed_data = (sgx_sealed_data_t *)malloc(sealedDataSize);

    if (sealedDataSize != sgx_fread(p_sealed_data, 1, sealedDataSize, key_file)) {
       mbedtls_printf("ERROR: secure file read failed. file is likely corrupted\n");
       if (p_sealed_data) 
         free(p_sealed_data);
       return SGX_ERROR_UNEXPECTED;
    ...
    err = sgx_unseal_data((const sgx_sealed_data_t *)p_sealed_data, NULL, NULL, plain_text, plain_text_len);
    if (err != SGX_SUCCESS) {
       mbedtls_printf("ERROR: unseal data failed\n");
       if (p_sealed_data) 
         free(p_sealed_data);
         return err;
    }
...
int seal_to_file(uint8_t* plain_text, uint32_t plain_text_len, const char *filename)
{
    sgx_status_t err = SGX_ERROR_UNEXPECTED;
    ...
    uint32_t sealedDataSize = sgx_calc_sealed_data_size(0,plain_text_len);
    sgx_sealed_data_t *p_sealed_data = (sgx_sealed_data_t *)malloc(sealedDataSize);

    err = sgx_seal_data_ex(SGX_KEYPOLICY_MRENCLAVE, attribute_mask, TSEAL_DEFAULT_MISCMASK,0,
                           NULL, plain_text_len, plain_text, sealedDataSize, p_sealed_data);

    if (err != SGX_SUCCESS) {
         mbedtls_printf("ERROR: sgx_seal_data_ex returned %d: %s\n",err, strerror(err));
         return err;
    }

   SGX_FILE* key_file = NULL;
   key_file  = sgx_fopen_auto_key(filename, "w");

   if (key_file == NULL) { //could not open key file for writing
       mbedtls_printf("ERROR: secure file %s open for write failed\n",filename);
       if (p_sealed_data) 
         free(p_sealed_data);
       return SGX_ERROR_UNEXPECTED;
   }
   ...
   if (sizeof(uint32_t) != sgx_fwrite(&sealedDataSize, sizeof(char), sizeof(uint32_t), key_file)) {
              mbedtls_printf("ERROR: file write failed. Likely permission or filessytem issue"); 
              if (p_sealed_data) 
                  free(p_sealed_data);
              return SGX_ERROR_UNEXPECTED;
      }
   // Now write the key to protected file
   if (sealedDataSize != sgx_fwrite((void *)p_sealed_data, sizeof(char), sealedDataSize, key_file)) {
              mbedtls_printf("ERROR: file write failed. Likely permission or filessytem issue"); 
              if (p_sealed_data) 
                  free(p_sealed_data);
              return SGX_ERROR_UNEXPECTED;
