...
decode_msisdn_ie_t(uint8_t *buf, msisdn_ie_t *val)
{
	...
	val->header.len = ( val->header.len >= BINARY_MSISDN_LEN ) ? BINARY_MSISDN_LEN -1 : val->header.len;
	val->msisdn[0] = val->header.len;
	memcpy(&val->msisdn[1], (uint8_t * )buf + count, val->header.len);
	count += val->header.len;
 ...
 */
static int
decode_uli_info_ie_t(uint8_t *buf, uli_info_ie_t *val)
{
	uint16_t count = 0;
	decode_ie_header_t(buf, &(val->header), IE_HEADER_SIZE);
	count += IE_HEADER_SIZE;

	memcpy(val->uli_info, (uint8_t * )buf + count, val->header.len);
	count += val->header.len;
...
{
	uint16_t count,bytes,pcoBytes;
	pco_flags_t flags;
	proto_cont_t protoCont;
	ppp_ipcp_t pppIpcp;
	count = bytes = pcoBytes = 0;
	...
	/* Adjust pcoBytes */
	pcoBytes = count - IE_HEADER_SIZE - 1;

	/* Copy the flags */
	memcpy(&flags, (uint8_t * )buf + bytes - 1, sizeof(pco_flags_t));

	/* Copy the protocol/containter id & len */
	memcpy(&protoCont, (uint8_t * )buf + bytes,  sizeof(proto_cont_t));

	/* Populate val */
	val->flags.conf_proto = flags.conf_proto;
	val->flags.spare = flags.spare;
	val->flags.ext   = flags.ext;
	val->prtCnt.proto_cont_id = ntohs(protoCont.proto_cont_id);
	val->prtCnt.proto_cont_len = protoCont.proto_cont_len;

	/* Already read first protocol or container id, so decrementing the length
	 * accordingly */
	pcoBytes -= (sizeof(proto_cont_t) + protoCont.proto_cont_len);
	/* Go to the IPCP block offset */
	while (ntohs(protoCont.proto_cont_id) != IPCP_PROTO_CONT_ID
				&& (pcoBytes > 0))
	{
		bytes += sizeof(proto_cont_t) + protoCont.proto_cont_len;
		memset(&protoCont, 0, sizeof(proto_cont_t));
		memcpy(&protoCont, (uint8_t * )buf + bytes, sizeof(proto_cont_t));
		pcoBytes -= (sizeof(proto_cont_t) + protoCont.proto_cont_len);
	}

	/* Check if Primary/Secondary DNS is requested */
	memcpy(&pppIpcp, (uint8_t *)buf + bytes + 3, sizeof(ppp_ipcp_t));

	if (pppIpcp.primary_dns_type == PRIMARY_DNS_IP)
	{
		/* Populate val with DNS IPs */
		val->ipcp.code = pppIpcp.code;
		val->ipcp.identifier = pppIpcp.identifier;
		val->ipcp.len = pppIpcp.len;

		val->ipcp.primary_dns_type = pppIpcp.primary_dns_type;
		val->ipcp.primary_dns_length = pppIpcp.primary_dns_length;
		val->ipcp.primary_dns_ip.s_addr = ntohl(pppIpcp.primary_dns_ip.s_addr);

		val->ipcp.secondary_dns_type = pppIpcp.secondary_dns_type;
		val->ipcp.secondary_dns_length = pppIpcp.secondary_dns_length;
		val->ipcp.secondary_dns_ip.s_addr = ntohl(pppIpcp.secondary_dns_ip.s_addr);
	}
	...
	while (count < msg_len) {
		...
		if (ie_header->type == IE_IMSI &&
			...
			count += decode_mei_ie_t(msg + count, &cs_req->mei);
				...
				ie_header->instance == IE_INSTANCE_ZERO) {
			//Decode ULI Info as it is (in Octet string format for CDR's
			decode_uli_info_ie_t(msg + count, &cs_req->uli_info);
			//Decode ULI info into the ULI struct based on flags
			...
			count += decode_ambr_ie_t(msg + count,
				...
				ie_header->instance == IE_INSTANCE_ZERO) {
			count += decode_pco_ie_t(msg + count,
								 &cs_req->pco);
