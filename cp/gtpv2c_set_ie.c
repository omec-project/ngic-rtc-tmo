	...
	enum ie_instance instance)
{
	...
	cause_ie_ptr->cause_ie_hdr.cause_value = cause_val;
	cause_ie_ptr->cause_ie_hdr.pdn_connection_error = 0;
	cause_ie_ptr->cause_ie_hdr.bearer_context_error = 0;
	cause_ie_ptr->cause_ie_hdr.cause_source = 0;
	cause_ie_ptr->spare_1 = 0;
...
{
	set_ie_header(&ambr->header, IE_AMBR,
		instance, (2 * sizeof(uint32_t)));

	ambr->apn_ambr_ul = ul_ambr;
	ambr->apn_ambr_dl = dl_ambr;
...
{
	set_ie_header(&pco->header,IE_PCO, instance, sizeof(uint8_t));

	/* Fill up the PCO IE fields */
	pco->header.type = IE_PCO;
	pco->header.len = sizeof(pco_flags_t) + sizeof(proto_cont_t)
							+ sizeof(ppp_ipcp_t);
	pco->header.instance = cs_req_pco.header.instance;

	pco->flags.conf_proto = cs_req_pco.flags.conf_proto;
	pco->flags.spare = cs_req_pco.flags.spare;
	pco->flags.ext = cs_req_pco.flags.ext;

	pco->prtCnt.proto_cont_id = cs_req_pco.prtCnt.proto_cont_id;
	pco->prtCnt.proto_cont_len = cs_req_pco.prtCnt.proto_cont_len;

	pco->ipcp.code = IPCP_CODE_NAK;
	pco->ipcp.identifier =  cs_req_pco.ipcp.identifier;
	pco->ipcp.len = cs_req_pco.ipcp.len;

	/* Fill up the primary DNS IP */
	pco->ipcp.primary_dns_type = cs_req_pco.ipcp.primary_dns_type;
	pco->ipcp.primary_dns_length = cs_req_pco.ipcp.primary_dns_length;
	pco->ipcp.primary_dns_ip.s_addr = pdns_ip.s_addr;

	/* Fill up the secondary DNS IP */
	pco->ipcp.secondary_dns_type = cs_req_pco.ipcp.secondary_dns_type;
	pco->ipcp.secondary_dns_length =  cs_req_pco.ipcp.secondary_dns_length;
	pco->ipcp.secondary_dns_ip.s_addr = sdns_ip.s_addr;
