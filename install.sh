...
SERVICE=3
#Intel SGX REQUIRED- DO NOT DISABLE (=0)
SGX_SERVICE=1
...
step_2()
{
	...
	TEXT[4]="Install DPDK"
	FUNC[4]="install_dpdk"
	if [ "$SGX_SERVICE" -eq 1 ] && [ "$SERVICE" -ne 1 ]; then
	TEXT[5]="Download Intel(R) SGX SDK"
	FUNC[5]="download_linux_sgx"
	fi
}
...
download_dpdk_zip()
{
	...
	echo ""
	echo "Applying KNI Mbuf allloc patch for resolved DP freeze issue(VCCCCB-42).."
	patch $DPDK_DIR/lib/librte_kni/rte_kni.c $NGIC_DIR/patches/kni_mbuf_alloc.patch

	if [ $? -ne 0 ] ; then
		echo "Failed to apply KNI mbuf alloc patch, please check the errors."
		return
	else
		echo "KNI mbuf alloc patch successfully applied to dpdk."
	fi
...
{
	while true; do
		read -p "Do you want control-plane with Intel(R) SGX based CDR? " yn
		case $yn in
	...
	while true;do
		...
		case $opt in
			[1])	echo "Control Plane Settings"
				echo "Intel SGX enabled by default on CP"
				SERVICE=1
				SERVICE_NAME="CP"
				recom_memory=1024
...
step_3()
{
	TITLE="Download and Install (Optional Packages)"
	TEXT[1]="Download and Install Hyperscan"
	FUNC[1]="download_hyperscan"
}

download_hyperscan()
{
	while true; do
		read -p "Agree to download and install Hyperscan. Press (y/n) to continue? " yn
		case $yn in
			[Yy]* )
				touch .agree_opt
				break;;
