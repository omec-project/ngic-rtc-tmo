...
#define BINARY_MSISDN_LEN	10 /* Added extra byte to store '\0' for CDR calculations */
...
/*
 * Protocol Configuration Option(PCO) specific defines
 */
#define IPCP_PROTO_CONT_ID 0x8021
#define PRIMARY_DNS_IP     129
#define SECONDARY_DNS_IP   131
#define PRIMARY_NBNS_IP    130
#define SECONDARY_NBNS_IP  132
#define IPCP_CODE_NAK      3
...
typedef struct uli_info_ie_t {
	ie_header_t header;
	uint8_t uli_info[ULI_LEN];
} uli_info_ie_t;
...
/* PCO flags */
typedef struct pco_flags_t {
	uint8_t conf_proto :3;
	uint8_t spare :4;
	uint8_t ext :1;
} pco_flags_t;

/* PCO id & len */
typedef struct proto_cont_t {
	uint16_t proto_cont_id;
	uint8_t  proto_cont_len;
} proto_cont_t;

/* The PCO IPCP block */
typedef struct ppp_ipcp_t {
	uint8_t  code;
	uint8_t  identifier;
	uint16_t len ;

	/* Primary DNS request */
	uint8_t primary_dns_type;
	uint8_t primary_dns_length;
	struct  in_addr primary_dns_ip;

	/* Pecondary DNS request */
	uint8_t secondary_dns_type;
	uint8_t secondary_dns_length;
	struct in_addr secondary_dns_ip;
} ppp_ipcp_t;
